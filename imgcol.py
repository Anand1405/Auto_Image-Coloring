...
def visualize(model, data, save=True, count=0):
    model.net_G.eval()
    with torch.no_grad():
    ...
    plt.imshow(L[0][0].cpu(), cmap='gray')
    plt.axis('off')
    ...
    '''  
    if save:
        #fig.savefig(f"colorization_{count}.png")
        img = Image.fromarray((fake_imgs[0] * 255).astype(np.uint8))
...
class Window(Frame):
    ...
    def color(self):
        ...
        load = Image.open(f"test_{count}.png")
        load = load.resize((480, 360), Image.ANTIALIAS)
